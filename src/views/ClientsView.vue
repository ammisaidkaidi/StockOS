...
function goToOrders(clientId) { router.push({ path: '/sales-orders', query: { client: clientId } }) }
function tryDelete(id) { try { db.deleteClient(id) } catch(e) { alert(e.message) } }
</script>
  ...
  <div>
    <div class="flex items-center justify-between mb-5">
      ...
      <div class="flex gap-2">
        <button class="btn btn-ghost btn-sm" @click="db.exportCSV('clients')">CSV</button>
        <button class="btn btn-primary" @click="modal.open('client')">+ New Client</button>
      </div>
    </div>
    <div class="card p-0 overflow-hidden">
          ...
          <tbody>
            <tr v-if="!filtered.length">
              <td colspan="9" style="text-align:center;color:var(--text3);padding:32px">
                No clients. <span style="color:var(--accent);cursor:pointer" @click="modal.open('client')">Add one</span>
              </td>
            </tr>
            <tr v-for="c in filtered" :key="c.id">
              ...
              <td><span class="badge b-blue" style="cursor:pointer" @click="goToOrders(c.id)">{{ db.salesOrders.filter(o=>o.client_id===c.id).length }}</span></td>
              <td><span class="badge" :class="c.is_active ? 'b-green' : 'b-gray'">{{ c.is_active ? 'active' : 'inactive' }}</span></td>
              <td><div class="flex gap-1">
                <button class="btn btn-ghost btn-sm" @click="modal.open('client', c)">Edit</button>
      ...
      <div class="flex items-center px-4 py-3 border-t" style="border-color:var(--border)">
        <span style="font-size:12px;color:var(--text2)">{{ filtered.length }} clients</span>
      </div>
    </div>
  </div>
</template>
