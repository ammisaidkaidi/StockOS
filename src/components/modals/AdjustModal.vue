...
const typeOptions   = [{ value:'in',label:'Stock In (+)' },{ value:'out',label:'Stock Out (−)' },{ value:'adjustment',label:'Adjustment (set)' }]
...
function save() {
  ...
  if (form.qty <= 0)    return alert('Quantity must be greater than 0')
  db.adjustStock({ ...form })
  emit('close')
}
</script>
...
<template>
  <ModalShell title="Stock Movement" sub="Record a stock in, out, or adjustment" @close="emit('close')" @save="save">
    <div class="space-y-4">
      <div class="input-wrap">
        <label>Product *</label>
        <SearchableSelect :options="productOptions" v-model="form.product_id" placeholder="Select product…" :search-mode="settings.searchMode"/>
      </div>
      <div class="grid grid-cols-2 gap-4">
        <div class="input-wrap">
          <label>Movement Type</label>
          <SearchableSelect :options="typeOptions" v-model="form.type" :search-mode="settings.searchMode" @update:modelValue="onTypeChange"/>
        </div>
        <div class="input-wrap">
        ...
        <div class="input-wrap">
          <label>Reason</label>
          <SearchableSelect :options="reasonOptions" v-model="form.reason" :search-mode="settings.searchMode"/>
        </div>
        <div class="input-wrap">
