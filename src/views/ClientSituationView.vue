...
// ── Filters ────────────────────────────────────────────────────────────────────
const f = ref({
  clientId: '', endCustomerId: '', paymentStatus: '',
...
const rows = computed(() => {
  return db.salesOrders
    ...
    .filter(so => {
      const v = f.value
      if (v.clientId       && so.client_id !== v.clientId)           return false
...
function doExport(fmt) {
  if (fmt==='csv')  exportCSV(rows.value,'client-situation.csv')
  if (fmt==='json') exportJSON(rows.value,'client-situation.json')
  if (fmt==='xlsx') exportExcel(rows.value,'client-situation.xlsx','Client Situation')
  if (fmt==='html') exportHTML(rows.value,'client-situation.html','Client Situation Report')
}
</script>
  ...
  <div>
    ...
    <div class="card p-5 mb-4">
      ...
      <div class="grid grid-cols-3 gap-4">
        ...
        <div class="input-wrap">
          <label>Client</label>
          <SearchableSelect :options="clientOptions" v-model="f.clientId" :search-mode="settings.searchMode"/>
        </div>
        <div class="input-wrap">
    ...
    <div class="card p-0 overflow-hidden">
      <div class="tbl-wrap">
        <table>
          <thead><tr>
            <th>SO#</th><th>Client</th><th>End Customer</th><th>Order Date</th>
