...
const topClients = computed(() => {
  return db.clients
    .map(c => ({ ...c, orders: db.salesOrders.filter(o => o.client_id === c.id).length }))
    .filter(c => c.orders > 0)
...
<template>
  <div>
      ...
      <div class="card p-5">
        ...
        <div class="space-y-3">
          ...
          <div style="border-top:1px solid var(--border);padding-top:12px">
            <div class="flex justify-between items-center">
              <span style="color:var(--text2)">Total Clients</span>
              <b class="font-mono" style="color:var(--accent2)">{{ db.clients.length }}</b>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- Row 3: top clients + most moved -->
    <div class="grid grid-cols-2 gap-4">

      <!-- Top clients -->
      <div class="card p-5">
        <h3 class="font-display font-semibold mb-4">Top Clients by Orders</h3>
        <div v-if="!topClients.length" style="color:var(--text3);font-size:13px">No sales data yet.</div>
        <div class="space-y-3">
          <div v-for="c in topClients" :key="c.id" class="flex items-center gap-3">
            ...
            <div class="stock-bar" style="width:80px">
              <div class="stock-bar-fill"
                :style="`width:${topClients[0]?.orders ? c.orders/topClients[0].orders*100 : 0}%;background:var(--accent2)`"/>
            </div>
