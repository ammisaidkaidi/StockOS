...
const search = ref('')
const allRows = computed(() => [...db.sPayments].reverse())
const filtered = useTableSearch(allRows, [
  r => db.getClient(r.client_id)?.full_name || '',
  r => db.salesOrders.find(o => o.id === r.order_id)?.so_number || '',
  'notes'
], search)

const payStatus = p => {
  const so = db.salesOrders.find(o => o.id === p.order_id)
  if (!so) return null
  const paid = db.soPaymentTotal(so.id)
  if (paid >= so.total) return { label:'Paid', cls:'b-green' }
...
<template>
  <div>
    <div class="flex items-center justify-between mb-5">
      ...
      <button class="btn btn-primary" @click="modal.open('s_payment')">+ New Payment</button>
    </div>
    <div class="card p-0 overflow-hidden">
      <div class="tbl-wrap">
        <table>
          <thead><tr><th>Date</th><th>Client</th><th>Order</th><th>Amount</th><th>Status</th><th>Notes</th><th>Actions</th></tr></thead>
          <tbody>
            ...
            <tr v-for="p in filtered" :key="p.id">
              ...
              <td>{{ db.getClient(p.client_id)?.full_name || '—' }}</td>
              <td class="font-mono" style="color:var(--accent)">{{ db.salesOrders.find(o=>o.id===p.order_id)?.so_number || '—' }}</td>
              ...
              <td><div class="flex gap-1">
                <button class="btn btn-ghost btn-sm" @click="modal.open('s_payment', p)">Edit</button>
                <button class="btn btn-danger btn-sm" @click="db.deleteSPayment(p.id)">Del</button>
              </div></td>
            </tr>
          </tbody>
        </table>
