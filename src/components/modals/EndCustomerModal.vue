...
const form = reactive({ full_name:'', phone:'', email:'', city:'', tax_id:'', address:'', notes:'', is_active:true, ...(modal.editData||{}) })

function save() {
  if (!form.full_name.trim()) return alert('Full name is required')
  db.saveEndCustomer({ ...form }, modal.editId); emit('close')
}
</script>
<template>
  <ModalShell :title="isEdit ? 'Edit End Customer' : 'New End Customer'" :edit-mode="isEdit" @close="emit('close')" @save="save">
    <div class="grid grid-cols-2 gap-4">
      <div class="input-wrap col-span-2"><label>Full Name *</label><input class="input" v-model="form.full_name" placeholder="End customer / institution name"/></div>
