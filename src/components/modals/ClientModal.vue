...
function save() {
  ...
  db.saveClient({ ...form }, modal.editId)
  emit('close')
}
</script>

<template>
  <ModalShell :title="isEdit ? 'Edit Client' : 'New Client'" :edit-mode="isEdit" @close="emit('close')" @save="save">
    <div class="grid grid-cols-2 gap-4">
      <div class="input-wrap col-span-2"><label>Full Name *</label><input class="input" v-model="form.full_name" placeholder="Client / company name"/></div>
      <div class="input-wrap"><label>Phone</label><input class="input" v-model="form.phone" placeholder="+213…"/></div>
      <div class="input-wrap"><label>Email</label><input class="input" v-model="form.email" placeholder="client@email.dz"/></div>
    ...
    <div class="flex items-center gap-3 mt-4">
      ...
      <span style="color:var(--text2);font-size:13px">Client Active</span>
    </div>
  </ModalShell>
</template>
