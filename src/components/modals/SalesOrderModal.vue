...
import { uuid, soSeq, fmtNum, fmtDate } from '@/utils/helpers'
...
const form = reactive({
  id: orderId, so_number: soSeq(), client_id: '', end_customer_id: '',
...
// Auto-focus client on mount
nextTick(() => { if (!isEdit.value) clientRef.value?.openDropdown() })
...
// ── Enter-key flow ────────────────────────────────────────────────────────────
function onClientSelected()     { persistHeader(); nextTick(() => deliveryDateRef.value?.focus()) }
...
const headerComplete = computed(() => !!form.client_id && !!form.so_number)
...
<template>
  ...
  <ModalShell :title="isEdit ? 'Edit Sales Order' : 'New Sales Order'"
    :edit-mode="isEdit" size="lg" @close="emit('close')" @save="save">
    ...
    <div class="grid grid-cols-2 gap-4 mb-5">
      ...
      <div class="input-wrap">
        <label>Client *</label>
        <SearchableSelect ref="clientRef" :options="clientOptions" v-model="form.client_id"
          placeholder="Select client…" :search-mode="settings.searchMode" clearable @selected="onClientSelected"/>
      </div>
      <div class="input-wrap">
